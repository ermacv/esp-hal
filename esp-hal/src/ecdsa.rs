...
// The normal HAL linker collects `.data.*` in SRAM. Applications that relocate
// ordinary data can collect the semantic `.data.critical.*` class in their
// interrupt-owned SRAM region before the generic `.data.*` rule.
#[unsafe(link_section = ".data.critical.ecdsa_irq")]
...
fn reset_peripheral() {
    crate::peripherals::HP_SYS_CLKRST::regs()
        .crypto_ctrl0()
        .modify(|_, w| w.crypto_ecdsa_rst_en().set_bit());
    crate::peripherals::HP_SYS_CLKRST::regs()
        .crypto_ctrl0()
        .modify(|_, w| {
            w.crypto_ecdsa_rst_en()
                .clear_bit()
                .crypto_rst_en()
                .clear_bit()
        });
