...
pub(crate) fn deinit_clocks() {
    // `enable_wifi(false)` closes the per-domain Wi-Fi gates. Keep the PMU baseline established by
    // `init_clocks` so a later radio reinitialization can safely access the modem register block.
}
